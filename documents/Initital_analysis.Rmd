...
qplot(words_exploded$N_total,  geom = 'boxplot' , 
      main = 'Distribution of word frequency accross the documentd',
      xlab = 'Number of times a word was mentioned') 
...
words_exploded[N_total>cutoff & Concentration>=2.5 , .(word, N_total, N_publication,
                                                  Concentration)][order(-Concentration)] %>% 
  unique() %>%
  knitr::kable(caption = paste("Words mentioned ",prettyNum(cutoff, big.mark = ',')," times (overall) and concentrated in one/few publications"))# showing words concentrated in few works, including also those appearing only in one publication (Concentration measure not available for those)
...
```{r evenly_sparse}
# Words appearing in all publications bu only once/ a few times  - potentially coming from metadata/title page) - not a concern, as they would have low frequency
...
set.seed(12345)
wordcloud2(words_total[N_total>100], color = sample(plasma(1000)) )
